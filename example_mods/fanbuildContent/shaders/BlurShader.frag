...
vec3 blur(sampler2D sp, vec2 uv, vec2 scale) {
    ...
    vec3 col = flixel_texture2D(sp, uv).rgb * weight;
    ...
    for (int x = 0; x <= samples / 2; ++x) {
        for (int y = 1; y <= samples / 2; ++y) {
            ...
            col += flixel_texture2D(sp, uv + scale * offset).rgb * weight;
            accum += weight;
...
void main() {
    ...
    gl_FragColor = vec4(blur(bitmap, uv, ps * amount), flixel_texture2D(bitmap,uv).a);
}